...
cancer_data<- select(cancer_data, -tumour_size) #Entfernt die Spalte tumour_size, da diese für die Krankheit Leukämie nicht relevant ist
...
previousdiseasinfam <- sample(0:3, 300, replace= TRUE)
#view(previousdiseasinfam)

#surroundings beschreibt, ob die Person einer radioaktiver Stahlung ausgesetzt war/ist z.B. durch Arbeit
#1=keinen Kontakt 2=mäßigen Kontakt 3=häufigen Kontakt
surroundings <- sample(1:3, 300, replace=TRUE,
...
alcohol <-         case_when(
                    cancer_data$age < 18 ~ sample(0:0, 300, replace= TRUE),
                    cancer_data$age >= 18 ~ sample(0:1, 300, replace=TRUE, prob=c(0.4,0.6)))
#view(alcohol)

bmi <- cancer_data$weight / ( (cancer_data$height / 100)^2) 
#view(bmi)
...
#gbt
gbt <- case_when(
  cancer_data$gender == 0 & data_comb$alcohol == 0 ~ sample(17:60, 300, replace= TRUE),
  cancer_data$gender == 1 & data_comb$alcohol == 0 ~ sample(20:63, 300, replace= TRUE),
  cancer_data$gender == 0 & data_comb$alcohol == 1 ~ sample(30:70, 300, replace= TRUE),
  cancer_data$gender == 1 & data_comb$alcohol == 1 ~ sample(35:73, 300, replace= TRUE),
  cancer_data$gender == 0 & data_comb$bmi < 25 ~ sample(17:60, 300, replace= TRUE),
  cancer_data$gender == 0 & data_comb$bmi >= 25 ~ sample(30:70, 300, replace= TRUE),
  cancer_data$gender == 1 & data_comb$bmi < 25 ~ sample(20:63, 300, replace= TRUE),
  cancer_data$gender == 1 & data_comb$bmi >= 25 ~ sample(35:73, 300, replace= TRUE),)
#view(gbt)

#eosinphils
#eosinphils <- sample(0.03:0.075, 300, replace= TRUE)
eosinphils <- runif(300, min=0.03, max= 0.075)
#view(eosinphils)

#monocytes
monocytes <- runif(300, min=0.03, max= 0.1)
#view(monocytes)

#lymphocytes
lymphocytes <- runif(n=300, min = 0.10, max = 0.57)
#view(lymphocytes)

#neutrophiles
neutrophiles <- 1-lymphocytes - eosinphils - monocytes

#wbc
#wir haben erst ein 10faches der gewünschten Zufallszahlen generiert, diese dann durch 10 geteilt, um eine Dezimalzahl zu bekommen
#Diese Variablen wurden dann bei case_when für den wbc-Wert eingesetzt
...
wbc <-   case_when(
  data_comb$age < 18 ~ wbcfirstround,
  data_comb$age >= 18 & data_comb$smoking==1 ~ wbcsecondround,
  data_comb$age >= 18 & data_comb$smoking==0 ~ wbcthirdround)
...
hemoglobine <-  case_when(
  data_comb$smoking ==1 & data_comb$gender==0 & data_comb$age >=18 ~ hemofirstround,
  data_comb$smoking ==1 & data_comb$gender==1 & data_comb$age >=18 ~ hemosecondround,
  data_comb$smoking ==0 & data_comb$gender==0 & data_comb$age >=18 ~ hemothirdround, 
  data_comb$smoking ==0 & data_comb$gender==1 & data_comb$age >=18 ~ hemofourthround,
  data_comb$age < 18 ~ hemofifthround   )
...
creatinine <- case_when(
  data_comb$gender==0 ~creafirstround,
  data_comb$gender==1 ~ creasecondround)
...
rbc <- case_when(
  data_comb$smoking ==1 & data_comb$gender==0 & data_comb$age >=18 ~ rbcfirstround,
  data_comb$smoking ==1 & data_comb$gender==1 & data_comb$age >=18 ~ rbcsecoundround,
  data_comb$smoking ==0 & data_comb$gender==0 & data_comb$age >=18 ~ rbcthirdround,
  data_comb$smoking ==0 & data_comb$gender==1 & data_comb$age >=18 ~ rbcfourthround,
  data_comb$age < 18 ~rbcfifthround )


#Datensätze zusammenfügen
#https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/cbind
data_comb <- cbind(cancer_data, previousdiseasinfam, surroundings, 
                   alcohol, bmi,diagnose, gbt, eosinphils, monocytes,lymphocytes, neutrophiles, 
                   hemoglobine, creatinine, rbc,  wbc)
view(data_comb)
